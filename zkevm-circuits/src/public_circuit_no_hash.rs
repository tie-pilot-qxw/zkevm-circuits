...
mod test {
    ...
    #[derive(Clone, Default, Debug)]
    pub struct PublicTestCircuit<F: Field, const MAX_NUM_ROW: usize>(PublicCircuit<F, MAX_NUM_ROW>);
    #[derive(Clone)]
    pub struct PublicTestCircuitConfig<F: Field> {
        pub public_circuit_config: PublicCircuitConfig<F>,
    ...
    fn test_public_circuit(witness: Witness) -> MockProver<Fp> {
        ...
        let circuit = PublicTestCircuit::<Fp, MAX_NUM_ROW>::new(witness);
