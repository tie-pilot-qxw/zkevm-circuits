...
impl Witness {
    ...
    pub fn new(chunk_data: &ChunkData) -> Self {
        ...
        // fill in cnt and length in public
        // some public rows generated by opcode gadgets
        // cnt=0 is padding row
    ...
    // fill in hash, cnt and length in public， and return public hash inputs
    pub fn fill_public_cnt_len<F: Field>(&mut self) {
        // fill cnt、length、hash
        let (num_padding_begin, _) = PublicCircuit::<F>::unusable_rows();
        let mut cnt = U256::zero();
        let length = self.public.len() - num_padding_begin;
        for row_cur in self.public.iter_mut() {
