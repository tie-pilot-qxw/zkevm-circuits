    ...
    ExecutionGadget<F, NUM_STATE_HI_COL, NUM_STATE_LO_COL> for Call3Gadget<F>
{
    ...
    fn get_constraints(
        ...
        let prev_is_call_2 = config.execution_state_selector.selector(
            meta,
            ExecutionState::CALL_2 as usize,
            Rotation(-1 * NUM_ROW as i32),
        );
        constraints.extend([("prev state is CALL_2".into(), prev_is_call_2 - 1.expr())]);

        let next_is_call_4 = config.execution_state_selector.selector(
            meta,
            ExecutionState::CALL_4 as usize,
            Rotation(super::call_4::NUM_ROW as i32),
        );
        let next_call_4_cnt_is_zero = config
            .cnt_is_zero
            .expr_at(meta, Rotation(super::call_4::NUM_ROW as i32));
        constraints.extend([(
            "next state is CALL_4".into(),
            next_call_4_cnt_is_zero * next_is_call_4 - 1.expr(),
        )]);
