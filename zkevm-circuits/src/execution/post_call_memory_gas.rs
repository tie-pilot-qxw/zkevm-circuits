...
use crate::execution::ExecutionState::POST_CALL;
use crate::execution::{
    post_call, Auxiliary, AuxiliaryOutcome, CoreSinglePurposeOutcome, ExecStateTransition,
...
/// post_call_memory_gas 前一个指令为call_3, 后一个指令为post_call
/// post_call_memory_gas 需要4个栈元素：
...
pub struct PostCallMemoryGasGadget<F: Field> {
    _marker: PhantomData<F>,
}
impl<F: Field, const NUM_STATE_HI_COL: usize, const NUM_STATE_LO_COL: usize>
    ExecutionGadget<F, NUM_STATE_HI_COL, NUM_STATE_LO_COL> for PostCallMemoryGasGadget<F>
{
    ...
    fn get_constraints(
        ...
        constraints.extend([
            ...
            (
                "lt_1 is bool".to_string(),
                lt_1.clone() * (1.expr() - lt_1.clone()),
            ),
            // Max(cur_memory_word_size, max_word_size) = next_word_size
            // tag: MemoryExpansion
            ...
            (
                "lt_2 is bool".to_string(),
                lt_2.clone() * (1.expr() - lt_2.clone()),
            ),
            // cur_memory_word_size * cur_memory_word_size / 512 = curr_quad_memory_cost
            // tag: U64DIV
        ...
        constraints.extend(config.get_exec_state_constraints(
            ...
            ExecStateTransition::new(
                ...
                NUM_ROW,
                vec![(POST_CALL, post_call::NUM_ROW, None)],
                None,
            ),
        ));
    ...
    fn get_lookups(
        ...
        vec![
            (
                "post_call_memory_gas stack read args_offset".into(),
                stack_lookup_0,
            ),
            (
                "post_call_memory_gas stack read args_length".into(),
                stack_lookup_1,
            ),
            (
                "post_call_memory_gas stack read ret_offset".into(),
                stack_lookup_2,
            ),
            (
                "post_call_memory_gas stack read ret_length".into(),
                stack_lookup_3,
            ),
            ("post_call_memory_gas args_div".into(), args_div),
            ("post_call_memory_gas max_word_size".into(), max_word_size),
    ...
    fn gen_witness(&self, trace: &GethExecStep, current_state: &mut WitnessExecHelper) -> Witness {
        ...
        let mut core_row_0 = ExecutionState::POST_CALL_MEMORY_GAS.into_exec_state_core_row(
            trace,
            current_state,
            NUM_STATE_HI_COL,
            NUM_STATE_LO_COL,
...
pub(crate) fn new<F: Field, const NUM_STATE_HI_COL: usize, const NUM_STATE_LO_COL: usize>(
    ...
    Box::new(PostCallMemoryGasGadget {
        _marker: PhantomData,
    })
}
...
mod test {
    ...
    fn assign_and_constraint() {
        ...
        let padding_end_row = |current_state| {
            let row = ExecutionState::POST_CALL.into_exec_state_core_row(
                &trace,
                current_state,
                NUM_STATE_HI_COL,
                NUM_STATE_LO_COL,
