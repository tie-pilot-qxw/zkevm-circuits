    ...
    ExecutionGadget<F, NUM_STATE_HI_COL, NUM_STATE_LO_COL> for SignextendGadget<F>
{
    ...
    fn get_constraints(
        ...
        let auxiliary_delta = AuxiliaryOutcome {
            ...
            gas_left: ExpressionOutcome::Delta(-OpcodeId::SIGNEXTEND.constant_gas_cost().expr()),
...
mod test {
    ...
    fn run(stack: Stack, stack_top: U256) {
        ...
        let gas_left_before_exec =
            current_state.gas_left + OpcodeId::SIGNEXTEND.constant_gas_cost();
        let mut trace = prepare_trace_step!(0, OpcodeId::SIGNEXTEND, stack);
