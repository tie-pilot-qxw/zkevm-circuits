...
pub struct SgtGadget<F: Field> {
    _marker: PhantomData<F>,
}

impl<F: Field, const NUM_STATE_HI_COL: usize, const NUM_STATE_LO_COL: usize>
    ExecutionGadget<F, NUM_STATE_HI_COL, NUM_STATE_LO_COL> for SgtGadget<F>
{
    ...
    fn get_constraints(
        ...
        constraints.extend((0..4).map(|i| {
            ...
            (
                ...
                stack_operands[i].clone() - arithmetic_operands_full[f_i].clone(),
            )
        }));
        ...
        constraints.extend([
            ...
            (
                "result in state push lo = operand[6] carry_hi in arithmetic".into(),
                stack_operands[5].clone() - arithmetic_operands_full[6].clone(),
            ),
            ("opcode".into(), opcode - OpcodeId::SGT.as_u8().expr()),
            (
                "arithmetic tag".into(),
                tag - (arithmetic::Tag::SltSgt as u8).expr(),
            ),
        ]);
        constraints
    }
    ...
    fn gen_witness(&self, trace: &GethExecStep, current_state: &mut WitnessExecHelper) -> Witness {
        assert_eq!(trace.op, OpcodeId::SGT);
        ...
        let (arithmetic, result) = operation::slt_sgt::gen_witness(vec![b, a]);
        ...
        let core_row_0 = ExecutionState::SGT.into_exec_state_core_row(
            trace,
            current_state,
            NUM_STATE_HI_COL,
            NUM_STATE_LO_COL,
...
mod test {
    ...
    fn assign_and_constraint() {
        ...
        let trace = prepare_trace_step!(0, OpcodeId::SGT, stack);
