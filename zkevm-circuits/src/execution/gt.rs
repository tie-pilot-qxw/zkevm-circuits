...
pub struct GtGadget<F: Field> {
    _marker: PhantomData<F>,
}
impl<F: Field, const NUM_STATE_HI_COL: usize, const NUM_STATE_LO_COL: usize>
    ExecutionGadget<F, NUM_STATE_HI_COL, NUM_STATE_LO_COL> for GtGadget<F>
{
    ...
    fn get_constraints(
        ...
        constraints.extend((0..4).map(|i| {
            ...
            (
                ...
                arithmetic_operands[i].clone() - arithmetic_operands_full[f_i].clone(),
            )
        }));
        ...
        constraints.extend([
            ("opcode".into(), opcode - OpcodeId::GT.as_u8().expr()),
            (
                "arithmetic tag".into(),
    ...
    fn gen_witness(&self, trace: &GethExecStep, current_state: &mut WitnessExecHelper) -> Witness {
        assert_eq!(trace.op, OpcodeId::GT);
        ...
        let (arithmetic, result) = operation::sub::gen_witness(vec![b, a]);
        ...
        let core_row_0 = ExecutionState::GT.into_exec_state_core_row(
            trace,
            current_state,
            NUM_STATE_HI_COL,
            NUM_STATE_LO_COL,
...
mod test {
    ...
    fn assign_and_constraint() {
        let stack = Stack::from_slice(&[5.into(), 19.into()]);
        ...
        let trace = prepare_trace_step!(0, OpcodeId::GT, stack);
