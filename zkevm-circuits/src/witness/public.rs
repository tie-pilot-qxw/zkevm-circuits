...
impl Row {
    ...
    pub fn from_chunk_data(chunk_data: &ChunkData) -> Result<Vec<Row>, anyhow::Error> {
        ...
        result.extend(Self::new_from_value(Row {
            tag: Tag::ChainId,
            ...
            for log_data in &block.logs {
                for log in log_data.logs.iter() {
                    ...
                    for i in 0..topic_num {
                        ...
                        result.extend(Self::new_from_value(Self::get_log_topic_row(
                            i as u8,
                            topic_hash,
                            block_tx_idx.into(),
                            log_index,
...
pub fn public_rows_hash_inputs<F: Field>(rows: &[Row]) -> Vec<u8> {
    ...
    let mut inputs = vec![vec![]; 6];
    for row in rows.iter().skip(num_padding_begin) {
