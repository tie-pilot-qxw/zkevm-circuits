...
impl Row {
            ...
            for (tx_idx, tx) in block.eth_block.transactions.iter().enumerate() {
                ...
                for (idx, byte) in tx.input.iter().enumerate() {
                    ...
                    result.push(Row {
                        ...
                        value_1_u8: Some(byte.clone().into()),
                        comments: [
                ...
                for log in log_data.logs.iter() {
                    ...
                    for (data_idx, data) in log.data.iter().enumerate() {
                        result.push(Row {
                            ...
                            value_1_u8: Some(data.clone().into()),
                            comments: [
    ...
    pub fn new_from_value(row: Row) -> Vec<Self> {
        let tag_u8 = &convert_u256_to_64_be_bytes(&U256::from(row.tag as u8))[16..32];
        let block_tx_idx_u8 =
            &convert_u256_to_64_be_bytes(&row.block_tx_idx.unwrap_or_default())[16..32];
        let value_0_u8 = &convert_u256_to_64_be_bytes(&row.value_0.unwrap_or_default())[16..32];
        let value_1_u8 = &convert_u256_to_64_be_bytes(&row.value_1.unwrap_or_default())[16..32];
        let value_2_u8 = &convert_u256_to_64_be_bytes(&row.value_2.unwrap_or_default())[16..32];
        let value_3_u8 = &convert_u256_to_64_be_bytes(&row.value_3.unwrap_or_default())[16..32];
...
pub fn witness_post_handle(witness: &mut Witness) {
    ...
    witness.public.sort_by(|row1, row2| {
        if row1.tag == Tag::TxLog && row2.tag == Tag::TxLog {
            // the rows with tag TxLog are sorted by public::LogTag
            row1.value_1.cmp(&row2.value_1)
        } else {
            row1.tag.cmp(&row2.tag)
        }
    });
    ...
    for _ in 0..witness.public.len() - num_padding_begin {
        let tag = witness.public[row_idx].tag;

        if (tag != Tag::TxCalldata && tag != Tag::TxLogData)
            || ((tag == Tag::TxCalldata || tag == Tag::TxLogData)
                && witness.public[row_idx].value_0.unwrap().is_zero())
        {
            let new_rows = Row::new_from_value(witness.public.remove(row_idx));
            witness.public.splice(row_idx..row_idx, new_rows);
            row_idx += PUBLIC_NUM_VALUES_U8_ROW; // +16
        } else {
            // do nothing
            row_idx += 1;
        }
...
pub fn public_rows_to_instance<F: Field>(rows: &[Row]) -> Vec<Vec<F>> {
    ...
    let public_inputs = public_rows_hash_inputs::<F>(rows);
    // calc public hash
