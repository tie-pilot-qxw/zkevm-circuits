...
use crate::util::{log2_ceil, rlc, Challenges};
    ...
    for (idx, chunk) in chunks.enumerate() {
        ...
        for round in 0..NUM_ROUNDS + 1 {
            ...
            if round < NUM_WORDS_TO_ABSORB {
                ...
                data_rlcs[NUM_BYTES_PER_WORD] = data_rlc.clone(); // Start at 0 or forward the previous value.
                for (idx, (byte, padding)) in input_bytes.iter().zip(paddings.iter()).enumerate() {
                    if !*padding {
                        let byte_value = Value::known(byte.value);
                        data_rlc = data_rlc.clone() * challenges.keccak_input() + byte_value;
                    }
                    data_rlcs[NUM_BYTES_PER_WORD - (idx + 1)] = data_rlc.clone();
                    // data_rlc_after_this_byte
                }
            } else {
                // In rounds without inputs, forward the previous value.
                data_rlcs[0] = data_rlc.clone();
            }
            ...
            if round != NUM_ROUNDS {
                ...
                let mut rho_pi_chi_cells: [[[Vec<Cell<F>>; 5]; 5]; 3] = std::array::from_fn(|_| {
...
pub(crate) fn calc_keccak_with_rlc<F: Field>(
    input: &[u8],
    challenges: &Challenges<Value<F>>,
